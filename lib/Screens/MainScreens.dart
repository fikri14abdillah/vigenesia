...
class MainScreens extends StatefulWidget {
  ...
  final String? email;
  final String? iduser;
  const MainScreens({Key? key, this.nama, this.email, this.iduser, String? id})
      : super(key: key);
...
class _MainScreensState extends State<MainScreens> {
  ...
  Future<dynamic> sendMotivasi(String isi) async {
    ...
    // print("test -> " + isi.toString());

    try {
      Response response = await dio.post("$baseurl/api/dev/Post_motivasi/",
          data: body,
          options: Options(
              contentType: Headers
  ...
  Future<List<MotivasiModel>> getData() async {
    var response = await dio.get(
        '$baseurl/api/Motivasi?iduser=${widget.iduser}'); // Ngambil by data
  ...
  Future<List<MotivasiModel>> getData2() async {
    var response =
        await dio.get('$baseurl/api/Motivasi/'); // Ngambil by ALL USER
